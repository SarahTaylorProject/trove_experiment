...
if (line_count > 0):
  greeting_string = "Thank you. Please wait while I collect {} random quotes into a poem.\n".format(line_count)
  tools_for_talking.say_something(text=greeting_string, speed=200)
  for i in range(1, line_count+1):
    print("Collecting quote {}".format(i))
  ...
  tools_for_talking.say_something(text=greeting_string, speed=200)
  greeting_string = "I used the following texts:"
  tools_for_talking.say_something(text=greeting_string, speed=200)
  
  metadata_list = []
  for quote in random_poetry_quotes:
    if quote[1] not in metadata_list:
      final_metadata = quote[1].replace("\n", "")
      tools_for_talking.say_something(text=final_metadata, speed=200)
