...
for current_song_line in input_lines:
    ...
    if ('call me' in current_song_line.lower()):
        random_word = 'Ishmael'
    else:
        word_list_tagged = nltk.pos_tag(word_list)
        last_word = word_list_tagged[-1]
        source_choice = random.choice(['names', 'tags', 'tags', 'corpus'])
        if (source_choice == 'tags'):
            #print(last_word)
            ...
            if (choice_list):
                ...
                random_word = random.choice(corpus_common_words)
        elif (source_choice == 'corpus'):
            random_word = random.choice(corpus_words)
        elif (source_choice == 'names'):
            random_word = random.choice(['Ahab', 'Moby Dick', 'Starbuck'])
